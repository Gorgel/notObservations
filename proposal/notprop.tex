...
%
\propabstract{We propose to use the Nordic Optical Telescope (NOT) to obtain deep surface spectroscopy
...
the galaxy's rotation curve. A light+dark mass profile will be fitted to the spectroscopic data in order
to determine the galaxy's stellar/dark matter components. The photometric data will be used
...
\piteln{314159265359}               % PI telephone number
...
%
\coinvest{Per Calissendorff}{106 91 Stockholm }  % {Name1, Institute1}   {Name2, Institute2} 
\coinvest{Simon Karlsson}{ 106 91 Stockholm}  % {Name3, Institute3}   etc
\coinvest{ }{ }
\coinvest{ }{ }
\coinvest{ }{ }
% 
...
\ntimer{5 H }      % Put requested time, in numbers, with unit (N or H), e.g 5 N
\nmonth{May }      % Put preferred month(s) here
\nmoonp{N }      % Put requested moon phase here: D, G or N
\nsemax{1.5 }      % Put maximum allowed seeing here: (0.7,1.0,1.5,N)
\nskyco{C }      % Put required photometric condition here
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{nightspage}
...
, Lyman-alpha forest measurements of distant galaxies (Viel, Matteo et al. 2009 MNRAS 399L , 39)
...
%
% Please type in the { } field below:
\techjust{Because the target NGC 5963 is of low surface brightness nature, we aim for fairly deep spectra (1.5-2h) in order to obtain reducible data from the outer skirts of the galaxy. We used spectral data from the \emph{Sloan Digital Sky Survey} (SDSS) in order to calculate the flux of the H$\alpha$ line which we will also look at. The flux was calculated from the equivalent width and continuum of the line and then converted into magnitude using Vega as a reference star. A somewhat average of the known flux from Vega at similar wavelengths as the 6565\AA we are interested in was used in the conversion and we get a magnitude for our line of interest as $m_{H\alpha} = 19.3$. Given an integration time of 2h we would achieve a \emph{Signal to Noise ratio} $(S/N)$ of $\approx 8.5$ according to the NOT's own calculator using the $\#8$ grism. In case that seeing is bad due to high air mass we would instead use the $\#7$ grism in order to boost our S/N very slightly, sacrificing a bit of a resolution. We also equip the ALFOSC instrument with both the 1.0" and the 1.3" slit on the same basis as with the grism choice. We assume that we may need up to 30 min of overhead time and thus need 2.5h in total for the spectroscopy. \\
...
\NOTconfig{A}{ALFOSC}{Spectro-long-slit}{1.0, 1.3}
\NOTconfig{A}{ALFOSC}{Spectro-long-slit}{Grism \#7, \#8}
